...
- **`bt` with no args** = planning entry point. From the trigger day (default Sunday, configurable via `core.wp_day`) until the weekly plan is done, runs weekly plan then daily plan — so a missed trigger day is caught up on the next `bt`. Other days, runs daily plan only. "Done this week" is keyed on the bt week (`core.week_start`, marker = the week's first day in `.wp_date`), not the ISO week, so a mid-week manual `bt wp` can't satisfy a week that starts later. If all done, shows Focus Log.
